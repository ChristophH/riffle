...
test_that('row_mean_grouped runs and returns expected output', {
  ...
  means <- riffle:::row_mean_grouped_dgcmatrix(matrix = pbmc, group_labels = grouping, shuffle = FALSE)
  ...
  means <- riffle:::row_mean_grouped_dgcmatrix(matrix = mat, group_labels = grouping, shuffle = FALSE)
