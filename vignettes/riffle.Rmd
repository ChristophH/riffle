...
# set the paths to resources that we need
PBMC_H5 <- 'vignette_resources/pbmc_10k_v3_filtered_feature_bc_matrix.h5'
PBMC_AZIMUTH <- 'vignette_resources/pbmc_10k_v3_azimuth_pred.tsv'
...
```{r}
counts <- Seurat::Read10X_h5(PBMC_H5)
predictions <- read.delim(PBMC_AZIMUTH, row.names = 1)
...
diff_mean_null <- sapply(1:R, function(i) {
  mean_r <- riffle:::row_gmean_grouped_dgcmatrix(matrix = tmp_counts, group = grp, eps = 1, shuffle = TRUE)
  mean_r[, 1] - mean_r[, 2]
})
...
```{r}
vst_out <- sctransform::vst(umi = counts, method = 'qpoisson', residual_type = 'none', return_cell_attr = TRUE, verbosity = 0)
counts_corrected <- sctransform::correct_counts(x = vst_out, umi = counts, verbosity = 0)
```

By default `riffle::diff_mean_test` applies some moderate pre-filtering  and
...
```{r}
bm <- bench::mark(
  de_res <- riffle::diff_mean_test(y = counts_corrected, 
...
The cells have been grouped into `r length(levels(s$seurat_clusters))` clusters. We are now going to use `riffle::diff_mean_test` to find the top cluster markers. For this we access the normalized counts that are in the `SCT` assay and the cluster labels.

```{r}
bm <- bench::mark(
  de_res <- riffle::diff_mean_test(y = GetAssayData(s, assay = 'SCT', slot = 'counts'), 
...
To speed up compilation of this vignette, we load the pre-calculated object.
```{r}
immune.combined.sct <- readRDS(file = 'vignette_resources/ifnb_integrated.Rds')
```
...
```{r}
bm <- bench::mark(
  de_res <- riffle::diff_mean_test_conserved(
...
mat <- riffle:::row_gmean_grouped_dgcmatrix(
...
> **_NOTE:_** This document was generated with `riffle` version `r packageVersion(pkg = 'riffle')`
